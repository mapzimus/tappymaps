...
assert(/pricing-price">\$0/.test(html), 'pricing page shows the free tier');
...
// Tappymaps is not a classroom product. These guard against the removed
// school-facing tier creeping back in through a copy-paste or a stale branch.
assert(!/classroom|worksheet|teacher|class code/i.test(html), 'no classroom/teacher surface in the app');
assert(!/classroom|worksheet|teacher/i.test(checkout), 'no classroom plan in checkout');
