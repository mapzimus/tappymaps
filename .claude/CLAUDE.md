...
- **Six playable games**: **Find the State** (Classic/Shuffle), **Stat Duel** (`kind:'duel'`), **State Capitals** (`kind:'capitals'`), **Neighbor Challenge** (`kind:'neighbors'`), **Speed Run** (50-state find), **State Shape ID** (`kind:'shape'` — a silhouette clue card rendered from the target's `getBBox`, tap it on the map). The engine dispatches on `game.kind` in `arcadeNextPrompt` / `arcadeOnTap` / the timeout path; absence of `kind` = the default single-tap find.
