...
- **Categories (50)**: 12 bundled static (`DRAFT_AREA`, `DRAFT_STATEHOOD`, `DRAFT_BORDERS`, computed name lengths ×2, plus the 2026-06-11 bank: `DRAFT_HIGHPOINT`, `DRAFT_MEAN_ELEV`, `DRAFT_COUNTIES`, `DRAFT_COASTLINE`, `DRAFT_EV`, `DRAFT_PRESIDENTS`, `DRAFT_PARKS`) + 38 generated from non-trend `DATA_MAP_DATASETS`. Census categories load through `fetchCensusData` (cached); if census is down a match still draws from the 12 static ones. **Pool categories** (coastline/presidents/parks) omit zero-value states — the engine's `isFinite` filter shrinks the pick pool and out-of-pool states get `.draft-state--out` (dimmed, unclickable). Reveal pacing accelerates per spec §7 (`240 + i*90` ms). The May-15 category source lists live at `docs/draft-categories/`; the improvement roadmap (Territory Draft, tiered rounds, AI difficulty — see `docs/superpowers/specs/2026-06-11-geodraft-improvement-design.md`).
- **Match shape**: 5 rounds, 3 picks/side alternating (player first), 10s pick timer (timeout = seeded random pick), values hidden until the reveal, totals decide, first to 3 wins. Ties award no pip.
- **Round start**: the category flashes BIG center-screen (`#draftCategoryPop` / `draftCatPop()`, ~1.15s locked beat before the first pick), not a corner text change.
- **Reveal drama** (`draftRevealRound`): picks animate highest-to-lowest while a live scoreline (`#dslYou`/`#dslAi`) ticks UP per pick (`dsl-bump`), winner side gets `dsl-win` emphasis + a verdict line. Match clinch (`draftShowComplete`) washes all 50 states in the winner's team colour before the result card.
